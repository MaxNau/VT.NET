...
namespace VT.NET.Endpoints
{
    ...
    public class VTIPs : VTEndpoint, IVTIPs
    {
        ...
        /// <returns>A task that represents the asynchronous operation, containing the <see cref="IPReport"/> for the specified IP address.</returns>
        public async Task<IPReport> GetReportAsync(string ipAddress, CancellationToken cancellationToken = default)
        {
            var validationResult = _ipAddressValidator.Validate(ipAddress);
            validationResult.ThrowIfAny();

            var result = await Self.GetAsync<VTResponse<IPReport>>($"ip_addresses/{ipAddress}", cancellationToken).ConfigureAwait(false);
            return result.Data;
        }
        ...
        /// <param name="cancellationToken">A <see cref="CancellationToken"/> to cancel the operation if needed.</param>
        /// <returns>A task that represents the asynchronous operation, containing the <see cref="VTAnalysis"/> of the rescan.</returns>
        public async Task<VTAnalysis> RescanAsync(string ipAddress, CancellationToken cancellationToken = default)
        {
            var validationResult = _ipAddressValidator.Validate(ipAddress);
            validationResult.ThrowIfAny();

            var result = await Self.PostAsync<VTResponse<VTAnalysis>>($"ip_addresses/{ipAddress}/analyse", null, cancellationToken).ConfigureAwait(false);
            return result.Data;
        }
    }
}
